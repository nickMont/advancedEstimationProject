...
load figs_and_mats\figs_mats_PES\HC_CL.mat
figure(11);clf;
figset
plot(xDstore(tDstore<=tDisc,1),xDstore(tDstore<=tDisc,2),'k--')
hold on
plot(stateOut(:,1),stateOut(:,2),'k-.')
plot(L*sin(0:.1:2*pi+.1),L*cos(0:.1:2*pi+.1),'k')
legend('Discretized','True','location','Southwest')
title('XY')
xlabel('X-displacement (nondimensionalized)')
ylabel('Y-displacement (nondimensionalized)')
figset
