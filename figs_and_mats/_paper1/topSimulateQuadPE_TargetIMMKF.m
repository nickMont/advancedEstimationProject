...
for t=t0:dt:tmax
    ...
    %preloading
    uEvaTempStack=cell(nmod*numTargets,1);
    RuStack=cell(nmod,1);
    uEvaTypeStack=cell(nmod*numTargets,1);
    ...
    for iT=1:numTargets
        ...
        for ij=1:nmod
            ...
            elseif strcmp(heurTypeStruc{ij},'ibr-linear-3d')
                ...
                uEvaTypeStack{(iT-1)*nmod+ij,1}='ibr-linear-3d';
            elseif strcmp(heurTypeStruc{ij},'ibr-linear-2d')
                ibrLinear2DScript;
                uEvaTemp=uEvaQuadOut;
                uEvaTypeStack{(iT-1)*nmod+ij,1}='ibr-linear-2d';
            elseif strcmp(heurTypeStruc{ij},'other')
                uEvaTemp=omega_hover*ones(4,1);
                Ru=1*eye(4);
                uEvaTypeStack{(iT-1)*nmod+ij,1}='hover';
            else
                error('Unrecognized control type in topSimulate, check ij index')
            end
%             ij
%             uEvaTemp
%             (iT-1)*nmod+ij
