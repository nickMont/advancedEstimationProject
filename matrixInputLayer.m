function layer = matrixInputLayer(varargin)
...
%                        Default: 'zerocenter'
%
%    'AverageImage'      The average image used for zero center normalization. 
%                        This can be a H-by-W-by-C array, a 1-by-1-by-C array
%                        of means per channel, or [], where H is the height, 
...
%       the average image for zero-center normalization to matrix ones.
%
%       layer = imageInputLayer([28 28], 'AverageImage', ones([28 28]));
%       
%       % Reset the average image to empty
%
%       layer.AverageImage = [];
%
%
...
%NOTE: This has only minor modifications from original MATLAB source
...
isValidSize = iIsRowVectorOfTwoOrThree(sz) && ...
...
type = cellstr(type);
tformarray = nnet.internal.cnn.layer.InputTransform.empty();
for i = 1:numel(type)
    tnew = nnet.internal.cnn.layer.InputTransformFactory.create(type{i}, imageSize);
