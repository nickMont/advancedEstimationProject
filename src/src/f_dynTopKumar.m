function [uPur,uEva,outputflag,uValP,uValE,Sminimax,Smisc] = f_dynTopKumar(Spur,Seva,gameState,vk,miscParams)
if nargin<=3
    vk=zeros(100,1);
end
...
if generateUsingUT
    ...
    [Cp0,Ce0] = generateCostMatricesKumar(SpurTemp,SevaTemp,gameStateTemp);
...
%     uEsum = uE0*w_mean_center;
    ...
    for ij=1:2*nx
        ...
        [Cp1,Ce1] = generateCostMatricesKumar(SpurTemp,SevaTemp,gameStateTemp);
...
elseif ~generateUsingUT
    [Cpur,Ceva,uP,uE]=generateCostMatricesKumar(Spur,Seva,gameState);
    [nP,nE]=size(Cpur);
end
