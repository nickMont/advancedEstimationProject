function [xCell,uCell] = generateStateMatFromPairingsNNvarDyn(pairings, Spur, Seva, gameState, network1, network2)
...
%         Re=Jparams.lowlevel.R;
        ...
        if this_numP==1
            qqrr=[diag(Qp); diag(Qe); diag(Rp); diag(Re);Spur.params.cd;Seva.params.cd]; %0.1 was tuning param in trainPE
        else
        ...
        for ik=1:gameState.kMax
            ...
            if this_numP==1
                ...
                ministate(1:nx) = feval(Spur.fname{purIndex},ministate(1:nx),uVec(1:nu),gameState.dt,zeros(nv,1),Spur.params);
                ministate(nx+1:end) = feval(Seva.fname{evaIndex},ministate(nx+1:end),uVec(nu+1:end),gameState.dt,zeros(nv,1),Seva.params);
