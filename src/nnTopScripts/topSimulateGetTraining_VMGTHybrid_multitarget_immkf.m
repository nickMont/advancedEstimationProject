...
muStore=cell(1,numNNiter);
xHatHistStore=cell(1,numNNiter);
...
if flagHasTarget
    ...
    targetPossibleStore{ikN}=xTargetPoss;
    targetTrueIndexStore{ikN}=xTargetIndexTrue;
...
nmod=length(modelsBox);
xhatE=repmat(xTrue(5:8),[1 nmod]);
PhatE=repmat(0.001*eye(4),[1 1 nmod]);
mu=1/nmod*ones(nmod,1);
muHist=mu;
stateEstHist=xTrue(5:8);
Rk=0.1*eye(4);
Mij=0.01*(nmod-1)*ones(nmod,nmod)+(1-0.01*(nmod-1))*eye(nmod);
[aMij,bMij]=size(Mij);
...
%         error('inc')
    ...
    zMeas=xTrue(5:8); %noiseless for IMMKF
    ...
    for ij=1:nmod
        ...
        if ctri==0
            controlTypeTemp='gt';
        elseif ctri==1
            controlTypeTemp='vmgt';
        elseif ctri==2
            controlTypeTemp='vm';
        else
            error('incorrect control type index')
        end
        
        % copy pasted from above with mild modification
        Spur_p.uMat={0};
        Seva_p.uMat={0};
        gameState_p.xPur=xTrue(1:4);
        gameState_p.xEva=xTrue(5:8);
        ...
        uhat=unit_vector(uhat);
        if strcmp(controlTypeTemp,'vmgt')
            for ik=1:length(uvec)
                Spur_p.uMat{ik}=upmax*uvec(ik)*uhat;
            end
        elseif strcmp(controlTypeTemp,'gt')
            for ik=1:length(utemp)
                Spur_p.uMat{ik}=utemp(:,ik);
            end
        elseif strcmp(controlTypeTemp,'vm')
            Spur_p.uMat{1}=uhat*upmax*0.8;
        else
            error('Invalid control type')
        end
        ...
        if flagHasTarget
            Spur_p.Jname='J_purTarget';
            Spur_p.Jparams.Q_target=QTargetPur;
            Spur_p.Jparams.x_target=xTargetPoss{nti};
            Seva_p.Jname='J_evaTarget';
            Seva_p.Jparams.Q_target=QTargetEva;
            Seva_p.Jparams.x_target=xTargetPoss{nti};
        end
        % Propagate to next time step
        [up,ue,flag,tmp1,tmp2]=f_dyn(Spur_p,Seva_p,gameState_p,zeros(4,1));
        if flag==0
            ...
            uEvaTemp=uEvaEst;
            Ru=0.01*eye(length(uEvaTemp));
            for ik=1:length(Seva_p.uMat)
                Ru=Ru+ue(ik)*(Seva_p.uMat{ik}(:,1)-uEvaTemp)*(Seva_p.uMat{ik}(:,1)-uEvaTemp)';
            end
        else
            uPurTrue=up;
            uEvaTemp=ue;
    ...
    for ij=1:nmod
        ...
        if min(eig(Paug))>0 %check for impossible models
            % Propagate
            [xhatEp1,Pp1]=ukfPropagate(xhatE(:,ij),PhatE(:,:,ij),uEvaMMF,RuMMF,dt,fdyn_trueNameE,Seva_p.params);
            
            % Measure
            [xhatE(:,ij),PhatE(:,:,ij),nu,Sk,Wk]=kfMeasure(xhatEp1,Pp1,zMeas,eye(4),Rk);
            Skt=triu(Sk); Skt=Skt'+Skt; Skt(1:5:end)=diag(Sk); %forces symmetry
            normpEval= mvnpdf(nu,zeros(4,1),Skt);
        else
            normpEval=1e-20;
        end
        if normpEval<=1e-8
...
if abs(ctrTypeR-modelsBox(mmInd,1))<=1e-5 && abs(xTargetIndexTrue-modelsBox(mmInd,2))<1e-5
    ...
    measStore{ikN}=zB;
    muStore{ikN}=muHist;
    xHatHistStore{ikN}=stateEstHist;
else
    ikN=ikN-1; %redo iteration
end
